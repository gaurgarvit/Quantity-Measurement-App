...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
    ...
    private void save(String operation, QuantityInputDTO input, QuantityMeasurementDTO dto, boolean error) {
        ...
        history.setResultString(dto.getResultString());
    ...
    public QuantityMeasurementDTO divide(QuantityInputDTO input) {
        if (input.getThatValue() == 0) throw new QuantityMeasurementException("Divide by zero");
        QuantityMeasurementDTO dto = new QuantityMeasurementDTO();
        double v1 = toBase(input.getThisUnit(), input.getThisValue());
        double v2 = toBase(input.getThatUnit(), input.getThatValue());
        double resultBase = v1 / v2;
    ...
    public List<OperationHistory> getByOperation(String op) { return repository.findByOperationIgnoreCase(op); }
    public List<OperationHistory> getByType(String type) { return repository.findByMeasurementTypeIgnoreCase(type); }
    public List<OperationHistory> getErrored() { return repository.findByErrorTrue(); }
    @Override public List<OperationHistory> getAll() { return repository.findAll(); }
    @Override public List<OperationHistory> getByUser() {
        String username = null;
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        if (auth != null && auth.isAuthenticated()) username = auth.getName();
        if (username != null && !username.equals("anonymousUser")) {
            User user = userRepository.findByUsername(username).orElse(null);
            if (user != null) return repository.findByUser(user);
        }
        return List.of();
    }
    public long count(String op) { return repository.countByOperationIgnoreCase(op); }
}
